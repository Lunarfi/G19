...
```{sql connection=my_connection, output.var="my_data"}
SELECT *
FROM product
```

```{r}
# Check if there are duplicate rows in the data frame
duplicate_rows <- duplicated(Product)

num_duplicates <- sum(duplicate_rows)

if (num_duplicates > 0) {
  print(paste("There are", num_duplicates, "duplicate rows in the data frame."))
} else {
